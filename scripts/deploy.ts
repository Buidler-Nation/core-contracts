...
async function main() {
  ...
  const bdnContract = await bdn.deploy();

  await bdnContract.deployed();

  const StakedBDN = await ethers.getContractFactory("StakedBDN");
  const sBDNContract = await StakedBDN.deploy();
  await sBDNContract.deployed();
  ...
  await bdnContract.setVault(treasury.address);

  await sBDNContract.initialize(stakingContract.address);
  ...
  await bdnContract.approve(treasury.address, constants.largeApproval);
}
