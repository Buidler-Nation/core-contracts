...
contract TAVCalculator {
    ...
    IBDN public immutable BDN;
    address[] assetManagers;
    address public _owner;
    mapping(address => bool) blacklistedAddress;

    constructor(address _bdn, address _treasury) {
        _owner = msg.sender;
    ...
    function calculateTAV() external view returns (uint256 _TAV) {
        uint256 bdnTotalSupply = BDN.totalSupply();
        ...
        _TAV = calculateTAVRatio(totalReserve, bdnTotalSupply);
    }

    function calculateTAVRatio(uint256 totalReserve, uint256 totalBDNSupply) internal pure returns(uint256) {
        return (FixedPoint.fraction(totalReserve, totalBDNSupply).decode112with18() / 1e9);
    }
