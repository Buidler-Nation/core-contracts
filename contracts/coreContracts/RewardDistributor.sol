...
contract RewardDistributor is Ownable {
    ...
    event RewardCycleCompleted(uint256 allocatedRewards, uint256 totalStakedBdn, uint8 rewardCycle);
    ...
    struct RewardCycle {
        ...
        uint32 endTimestamp;
        uint256 totalStakedBdnAmount;
        uint256 totalAllocatedRewards;
    }

    struct UserStakeInfo {
        uint256 stakedBdnAmount;
    ...
    address private _stakedBdnAddress;
    ...
    constructor(address stakingContract_, address stakedBdnAddress_) {
        require(stakingContract_ != address(0));
        require(stakedBdnAddress_ != address(0));
        ...
        _stakedBdnAddress = stakedBdnAddress_;
    }
    ...
    function completeRewardCycle(uint256 rewardAmount) external onlyOwner {
        ...
        rewardCycle.totalStakedBdnAmount = IERC20(_stakedBdnAddress).totalSupply();
        _rewardCycleMapping[currentRewardCycle] = rewardCycle;
        emit RewardCycleCompleted(rewardAmount, rewardCycle.totalStakedBdnAmount, currentRewardCycle);
        currentRewardCycle++;
    ...
    function stake(address to_, uint256 amount) external onlyStakingContract {
        updateStakeBdnBalance(to_, amount, true);
    }

    function unstake(address to_, uint256 amount) external onlyStakingContract {
        updateStakeBdnBalance(to_, amount, false);
    }
    ...
    function rewardsForACycle(address account_, uint8 rewardCycle_) public view returns(uint256) {
        ...
        uint256 stakedBdnPortion = calculateStakeBdnPortion(userStakeInfo.stakedBdnAmount, rewardCycle.totalStakedBdnAmount);

        return calculateRewards(investedTimeInCycle, stakedBdnPortion, rewardCycle.totalAllocatedRewards);
    }

    function updateStakeBdnBalance(address to_, uint256 amount, bool isStake) internal returns(uint256){
        ...
        if(isStake) {
            ...
            (
                ...
                userStakeInfo.stakedBdnAmount,
                amount
            );
            userStakeInfo.averageInvestedTime = uint32(tAVG.div(1e18));
            userStakeInfo.stakedBdnAmount = userStakeInfo.stakedBdnAmount.add(amount);
            ...
            userStakeInfo.stakedBdnAmount = userStakeInfo.stakedBdnAmount.sub(amount);
            _userStakeInfoToRewardCycleMapping[to_][currentRewardCycle] = userStakeInfo;
            if(userStakeInfo.stakedBdnAmount == 0){
    ...
    function updateBalanceBasedOnPreviousCycle(address to_) internal {
        if(_userActivityMapping[to_] != currentRewardCycle) {
            for (uint8 i=_userActivityMapping[to_]+1; i<=currentRewardCycle; i++){
                ...
                newUserStakeInfo.stakedBdnAmount = userStakeInfo.stakedBdnAmount;
    ...
    function calculateAverageTime(uint32 tAVG, uint32 rewardCycleStartTime, uint256 stakedBdnAmount, uint256 amount) internal view returns(uint256){
        uint256 stakeTimeValue = tAVG.mul(stakedBdnAmount);
        ...
        return calculateAverageTime(totalStakeTimeValue, stakedBdnAmount.add(amount));
    }
    ...
    function calculateStakeBdnPortion(uint256 stakeBdnAmount, uint256 totalStakedBdnAmount) internal pure returns(uint256) {
        return (FixedPoint.fraction(stakeBdnAmount, totalStakedBdnAmount).decode112with18() / 1e15).mul(1e15);
    }

    function calculateAverageTime(uint256 totalStakeTimeValue, uint256 totalStakedBdnAmount) internal pure returns(uint256) {
        return (FixedPoint.fraction(totalStakeTimeValue, totalStakedBdnAmount).decode112with18() / 1e15).mul(1e15);
    }

    function calculateRewards(uint256 investedTime, uint256 stakedBdnPortion, uint256 totalRewards) internal pure returns(uint256) {
        return (investedTime.mul(stakedBdnPortion).mul(totalRewards)).div(1e36);
    }
    ...
    function stakedBdnAddress() external view returns(address) {
        return _stakedBdnAddress;
    }
    ...
    function getTotalStakedBdnOfUserForACycle(address account_, uint8 rewardCycle) external view returns(uint256 amount_) {
        amount_ = _userStakeInfoToRewardCycleMapping[account_][rewardCycle].stakedBdnAmount;
    }

    function getTotalStakedBdnForACycle(uint8 rewardCycle) external view returns(uint256 amount_) {
        amount_ = _rewardCycleMapping[rewardCycle].totalStakedBdnAmount;
    }
