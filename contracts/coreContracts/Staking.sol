...
contract Staking is Ownable {
  ...
  event BdnStaked(address indexed account, uint256 amount);
  event BdnUnstaked(address indexed account, uint256 amount);
  ...
  function claim(address _to) external returns (uint256) {
    ...
    if (block.timestamp >= info.expiry && info.expiry != 0) {
      ...
      emit BdnStaked(_to, info.deposit);
  ...
  */
   ...
   */
  function contractBDNBalance() external view returns (uint256) {
