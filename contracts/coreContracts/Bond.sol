...
contract Bond is Ownable {
    ...
    struct Terms {
        ...
        uint256 minimumPayout; // minimum BDN that needs to be bonded for
    }

    // Info for bond holder
    struct BondInfo {
        uint256 payout; // BDN remaining to be paid
    ...
    IERC20 public immutable BDN; // token given as payment for bond
    IERC20 public principle; // token used to create bond
    ITreasury public immutable treasury; // mints BDN when receives principle
   ...
   */
    ...
    function setStaking(address _staking) external onlyOwner {
        ...
        BDN.approve(address(staking), 1e45);
    }
   ...
   */
    function deposit(
        ...
        require(payout >= minPayout(), 'Bond too small'); // must be > 0.01 BDN ( underflow protection )
        require(payout <= maxPayout(), 'Bond too large'); // size protection because there is no slippage

        // profits are calculated
        uint256 fee = payout.mul(terms.fee) / 100000;
        uint256 bdnToMint = payout.add(fee);
        ...
        treasury.deposit(_amount, address(principle), bdnToMint);

        if (fee != 0) {
            // fee is transferred to dao
            BDN.safeTransfer(DAO, fee);
        }
   ...
   */
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            BDN.safeTransfer(_recipient, _amount); // send payout
        } else {
   ...
   */
    function maxPayout() public view returns (uint256) {
        return (BDN.totalSupply().mul(terms.maxPayout) / 100000);
    }
   ...
   */
    ...
    /**
    *  @notice calculate current ratio of debt to BDN supply in 1e9 equivalent
    *  @return debtRatio_ uint
    */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = BDN.totalSupply();
   ...
   */
    ...
    /**
   *  @notice calculate amount of BDN available for claim by depositor
   *  @param _depositor address
   ...
   */
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or BDN) to the DAO
   *  @return bool
   */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != address(BDN), 'NAT');
